...
export default function RentalProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/listing/rental`);
      ...
      // Get static RENTAL products
      const staticRentalProducts = staticProducts
        .filter((product: any) => product.product_type === 'rental')
        .map((product: any) => ({
          ...product,
      ...
      const combined = [...transformedApiProducts, ...staticRentalProducts];
      ...
      setProducts(staticRentalProducts);
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Rental Products</h1>
          <p className="text-gray-600">Rent equipment for your projects</p>
          {!loading && (
            <p className="text-sm text-gray-500 mt-2">
              {products.length} product{products.length !== 1 ? 's' : ''} available for rent
            </p>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2 h-12 group-hover:text-purple-600">
          {product.name}
        </h3>
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xl font-bold text-purple-600">AED {price}</span>
        ...
        >
          ...
          {isInStock ? 'Book Now' : 'Unavailable'}
        </button>
      </div>
    </Link>
  );
