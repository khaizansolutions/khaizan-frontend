...
export default function RefurbishedProductDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (resolvedParams.slug.startsWith('static-')) {
        ...
        if (staticProduct && staticProduct.product_type === 'refurbished') {
        ...
        try {
          ...
          // Check if product exists and is REFURBISHED
          if (apiProduct && apiProduct.product_type === 'refurbished') {
  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-600 mb-8 text-sm md:text-base">The product you're looking for doesn't exist or is not a refurbished product.</p>
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="hidden md:block bg-white border-b">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <Link href="/refurbished" className="hover:text-green-600">Refurbished Products</Link>
            <span>/</span>
      ...
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-8">
        ...
        <Link
          href="/refurbished"
          className="inline-flex items-center gap-2 text-green-600 hover:underline mb-4 md:mb-6 text-sm md:text-base"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 bg-white rounded-lg shadow-lg p-4 md:p-8">
          ...
          <div>
            <div className="relative h-64 sm:h-80 md:h-96 lg:h-[500px] bg-gray-100 rounded-lg overflow-hidden mb-3 md:mb-4">
              ...
              {/* REFURBISHED Badge */}
              <div className="absolute top-2 md:top-4 left-2 md:left-4 bg-green-600 text-white px-3 py-1 rounded-full font-bold text-xs md:text-sm flex items-center gap-1">
                <Package size={14} />
                REFURBISHED
              </div>
              {product.discount && product.discount > 0 && (
                <div className="absolute top-2 md:top-4 right-2 md:right-4 bg-red-500 text-white px-2 md:px-3 py-1 rounded-full font-bold text-xs md:text-sm">
                  -{product.discount}%
                </div>
              )}
            </div>
            ...
            {product.images && product.images.length > 0 && (
              <div className="grid grid-cols-4 gap-2 md:gap-4">
                {product.images.map((img: any, index: number) => {
                  ...
                  return (
                    <div key={index} className="relative h-16 sm:h-20 md:h-24 bg-gray-100 rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-green-600 transition">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 md:gap-4 mb-3 md:mb-4 text-sm md:text-base">
              ...
              <span className="text-green-600 hover:underline cursor-pointer text-xs md:text-sm">
            ...
            <div className="mb-4 md:mb-6">
              ...
              {normalizedOriginalPrice && (
                <p className="text-xs md:text-sm text-green-600 mt-1 font-semibold">
                  Save AED {(normalizedOriginalPrice - price).toFixed(2)} - Great Value!
                </p>
              )}
            </div>
            ...
            <div className="bg-green-50 border-l-4 border-green-600 p-3 md:p-4 mb-4 md:mb-6">
              <h4 className="font-semibold text-green-800 mb-2 text-sm md:text-base">Why Buy Refurbished?</h4>
              <ul className="text-xs md:text-sm text-gray-700 space-y-1">
                <li>✓ Up to 50% savings compared to new</li>
                <li>✓ Fully tested and certified</li>
                <li>✓ Same warranty as new products</li>
            ...
            <div className="mb-4 md:mb-6">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                <div className="flex items-center border-2 border-gray-300 rounded-lg">
                  <button onClick={decrementQuantity} className="px-3 md:px-4 py-2 hover:bg-gray-100 transition text-lg md:text-xl">-</button>
                  <input
                    type="number"
                    value={quantity}
                    onChange={(e) => {
                      const val = parseInt(e.target.value)
                      if (val >= 1 && val <= stockCount) setQuantity(val)
                    }}
                    min={1}
                    max={stockCount}
                    className="w-12 md:w-16 text-center border-x-2 border-gray-300 py-2 focus:outline-none text-sm md:text-base"
                  />
                  <button onClick={incrementQuantity} className="px-3 md:px-4 py-2 hover:bg-gray-100 transition text-lg md:text-xl">+</button>
                </div>
            ...
            <div className="flex flex-col md:flex-row gap-2 md:gap-4 mb-6 md:mb-8">
              <button
                ...
                className="flex-1 bg-green-600 text-white py-3 md:py-4 rounded-lg hover:bg-green-700 transition flex items-center justify-center gap-2 text-sm md:text-lg font-semibold"
              >
            ...
            {/* Features Grid */}
            <div className="grid grid-cols-3 gap-2 md:gap-4 mb-6 md:mb-8 p-3 md:p-6 bg-green-50 rounded-lg">
              <div className="text-center">
                <Truck className="mx-auto mb-1 md:mb-2 text-green-600" size={24} />
              ...
              <div className="text-center">
                <Shield className="mx-auto mb-1 md:mb-2 text-green-600" size={24} />
              ...
              <div className="text-center">
                <RefreshCcw className="mx-auto mb-1 md:mb-2 text-green-600" size={24} />
            ...
            <div className="flex gap-4 md:gap-8 min-w-max">
              <button
                ...
                className={`pb-3 md:pb-4 text-sm md:text-base whitespace-nowrap ${
                  activeTab === 'features' ? 'border-b-2 border-green-600 text-green-600 font-semibold' : 'text-gray-600 hover:text-green-600'
                }`}
              >
                Features
              </button>
              <button
                ...
                className={`pb-3 md:pb-4 text-sm md:text-base whitespace-nowrap ${
                  activeTab === 'specifications' ? 'border-b-2 border-green-600 text-green-600 font-semibold' : 'text-gray-600 hover:text-green-600'
                }`}
              >
                Specifications
              </button>
              <button
                ...
                className={`pb-3 md:pb-4 text-sm md:text-base whitespace-nowrap ${
                  activeTab === 'reviews' ? 'border-b-2 border-green-600 text-green-600 font-semibold' : 'text-gray-600 hover:text-green-600'
                }`}
              >
              ...
              {product.features && product.features.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  {product.features.map((feature: string, index: number) => (
                    <div key={index} className="flex items-start gap-2 md:gap-3 p-3 md:p-4 bg-green-50 rounded-lg">
